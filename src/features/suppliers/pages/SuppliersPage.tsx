...
export const SuppliersPage: React.FC<SuppliersPageProps> = ({ suppliers, setSuppliers, assets }) => {
  const {
    isModalOpen,
    editingSup,
    openCreateModal,
    ...
    updateSupplier,
    deleteSupplier,
  } = useSuppliers(suppliers, setSuppliers);
  ...
  const handleSave = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const success = editingSup ? updateSupplier(editingSup.id, data) : createSupplier(data);
    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <button
          ...
          className="bg-indigo-600 text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-lg shadow-indigo-600/20 flex items-center"
        >
            ...
            <form onSubmit={handleSave} className="p-8 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    name="taxCode"
                    defaultValue={editingSup?.taxCode}
                    required
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    name="contactPerson"
                    defaultValue={editingSup?.contactPerson}
                    required
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500"
                  />
                </div>
              ...
              <div className="flex gap-3 pt-6">
                <button
                  type="submit"
                  className="flex-1 bg-indigo-600 text-white font-bold py-3 rounded-xl shadow-xl shadow-indigo-600/20 hover:bg-indigo-700"
                >
                  Xác nhận Lưu
                </button>
                <button
                  type="button"
                  onClick={closeModal}
                  className="px-6 py-3 border border-slate-200 font-bold rounded-xl text-slate-600"
                >
                  Hủy bỏ
                </button>
              </div>
